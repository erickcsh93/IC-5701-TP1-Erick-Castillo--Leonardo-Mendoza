	...
	nodo *creaNodoConPos (char *valor, int fil, int col){
		nodo *rama = (nodo*) malloc(sizeof(nodo));
		rama->str = (char *)malloc(strlen(valor)+1);
		rama->fil = fil;
		rama->col = col;
	...
	nodo *creaNodoRedEET(char *prod,nodo *arbol,char *token,char *tira){
		nodo* padre = creaNodo(prod);
		nodo* hijo = creaNodoToken(token,tira);
		//printf (">>>>>>>>>>> %s",arbol->hijos->node->str);
		if (arbol->hijos->node->str != ""){

			agregaHijo(arbol,hijo);
			return arbol;
		}
		else{
	...
	nodo *creaNodoRedAtr(char *atrb,nodo *val, int fil, int col){
		nodo* padre = creaNodoConPos("atributo", fil, col);
